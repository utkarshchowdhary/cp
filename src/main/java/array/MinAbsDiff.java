...
//        }
        ...
        for(int i = 0; i < n; ++i) {
            ...
            count[i][a[i]]++; // increment the frequency column associated with array value
        }
        ...
        for (int i = 0; i < m; i++) {
            ...
            for (int j = 0 ; j < max + 1; j++) {
                /*
                * if subtraction between the frequencies of end row index and one before start row index
                * or in case the range starts from beginning the frequencies of end row index are
                * greater than zero this implies the column index value must exists within the range
                * */
